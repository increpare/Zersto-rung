...
func löscheSpalte(b int, h int, raster [][]uint8, x int) {
	...
	if x < b-1 {
		//lösche nach oben zeigende Pfeilen von der unteren Zeile
		for j := 0; j < h; j++ {
			raster[x+1][j] = raster[x+1][j] &^ vL
		}
	}
	if x > 0 {
		//lösche nach oben zeigende Pfeilen von der unteren Zeile
		for j := 0; j < h; j++ {
			raster[x-1][j] = raster[x-1][j] &^ vR
		}
	}
}
...
func linksteZeile(b int, h int, höhepunkt int, raster [][]uint8) int {
	for i := 0; i < b; i++ {
		for j := h - 1; j >= höhepunkt; j-- {
			if raster[i][j] != 0 {
				return i
			}
		}
...
func rechsteZeile(b int, h int, höhepunkt int, raster [][]uint8) int {
	for i := b - 1; i >= 0; i-- {
		for j := h - 1; j >= höhepunkt; j-- {
			if raster[i][j] != 0 {
				return i
...
func losRaster(b int, h int, raster [][]uint8, bleibendezüge int, scores []int) {
	...
	var l = linksteZeile(b, h, gipfel, raster)
	var r = rechsteZeile(b, h, gipfel, raster)

	for i := l; i <= r; i++ {
		kopie := kopiereRaster(b, h, raster)
		löscheSpalte(b, h, kopie, i)
		vollFallenLassen(b, h, kopie)
